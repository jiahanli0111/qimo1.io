...
<%@ Page language="c#" Codebehind="JF_News_Main.aspx.cs" AutoEventWireup="false" Inherits="JFCmsEnterprise.manageNews.News.JF_News_Main" %>
	...
	<body topMargin="0">
		<FONT face="宋体">
			<form id="Form1" method="post" runat="server">
				<FIELDSET style="WIDTH: 100%" align="center"><LEGEND><B><%=CategoryName%></B></LEGEND>
					<TABLE id="Table6" cellSpacing="0" cellPadding="0" width="100%" align="center" border="0">
								...
								<table cellSpacing="0" cellPadding="0" width="99%" align="center" border="0">
									<tr>
										<td width="483" height="25">
											<INPUT class="button80" onClick="window.location='JF_EditNews.aspx?CategoryID=<%=CategoryID%>&amp;CategoryName=<%=CategoryName%>'" type="button" value="添加新闻">
											<INPUT class="button80" onClick="window.location='JF_News_Main.aspx?CategoryID=<%=CategoryID%>&amp;CategoryName=<%=CategoryName%>&amp;otype=unauditing'" type="button"value="未审核新闻">
											<INPUT class="button80" onClick="window.location='JF_News_Main.aspx?CategoryID=<%=CategoryID%>&amp;CategoryName=<%=CategoryName%>'" type="button"value="所有新闻">
											<INPUT class="button80" onClick="window.location='JF_UpdatePage.aspx?Action=UpCatAll&amp;CategoryID=<%=CategoryID%>&amp;CategoryName=<%=CategoryName%>'" type="button"value="更新本栏目">
											<INPUT class="button80" onClick="window.location='JF_UpdatePage.aspx?Action=UpIndex&amp;CategoryID=<%=CategoryID%>&amp;CategoryName=<%=CategoryName%>'" type="button"value="更新首页">										</td>
						...
						<TR class="category_menu">
							<TD width="61%" align="center" noWrap background="../images/tableheadingback.gif" style="HEIGHT: 18px">标题</TD>
							<TD width="1%" align="center" valign="top" noWrap background="../images/tableheadingback.gif" style="HEIGHT: 18px"><IMG src="../images/tableheadingsep.gif" width="6" height="17"></TD>
							<TD width="7%" noWrap background="../images/tableheadingback.gif" style="HEIGHT: 18px">录入用户</TD>
							...
							<TD noWrap width="9%" background="../images/tableheadingback.gif">录入时间</TD>
							...
							<TD noWrap width="7%" background="../images/tableheadingback.gif">评论</TD>
							<TD vAlign="top" noWrap width="1%" background="../images/tableheadingback.gif"><IMG height="17" src="../images/tableheadingsep.gif" width="6"></TD>
							<TD noWrap width="12%" background="../images/tableheadingback.gif">修改|删除</TD>
						</TR>
							...
							<ItemTemplate>
								<TR class="category_menu" onMouseOver="this.style.backgroundColor='#E6F5FA'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
									<TD align="left" noWrap style="TABLE-LAYOUT: fixed; WORD-BREAK: break-all; WORD-WRAP: break-word"><%#(bool)DataBinder.Eval(Container.DataItem,"TitleNews")?"<img src='../images/newstype/title.gif' align='absmiddle' title='标题新闻'>":""%><%#DataBinder.Eval(Container.DataItem,"PicPath").ToString()!=""?"<img src='../images/newstype/img.gif' align='absmiddle' title='图片新闻'>":""%><%#DataBinder.Eval(Container.DataItem,"Picdepict").ToString()!=""?"<img src='../images/newstype/depict.gif' align='absmiddle' title='摘要新闻'>":""%><%#(bool)DataBinder.Eval(Container.DataItem,"Recommend")?"<img src='../images/newstype/recommend.gif' align='absmiddle' title='推荐新闻'>":""%>
										<%#(bool)DataBinder.Eval(Container.DataItem,"TitleNews")?"<a href='"+DataBinder.Eval(Container.DataItem,"ObjUrl")+"' target=_blank>":"<a href='"+CategoryDomain+DataBinder.Eval(Container.DataItem,"DatePath")+"/"+DataBinder.Eval(Container.DataItem,"FileName")+"."+FileExt+"' target=_blank>"%>
									...
									<TD noWrap width="7%">
										<a href='javascript:' onclick='newwin("JF_ReviewManage.aspx?NewsID=<%#DataBinder.Eval(Container.DataItem,"NewsID")%>","review","700","620")'>
											评论管理</a>									</TD>
									<TD noWrap width="1%"></TD>
									<TD noWrap width="12%"><a href='JF_EditNews.aspx?CategoryID=<%=CategoryID%>&CategoryName=<%=CategoryName%>&NewsID=<%# DataBinder.Eval(Container.DataItem,"NewsID")%>'>修改</a>|<a href='?action=DelNews&CategoryID=<%=CategoryID%>&CategoryName=<%=CategoryName%>&NewsID=<%# DataBinder.Eval(Container.DataItem,"NewsID")%>&DatePath=<%# DataBinder.Eval(Container.DataItem,"DatePath")%>&FileName=<%# DataBinder.Eval(Container.DataItem,"FileName")%>' onClick="return Del()">删除</a>|
					...
					<table cellSpacing="0" cellPadding="0" width="100%" border="0">
						<tr vAlign="bottom" align="right">
							<td colSpan="2" height="27"><asp:dropdownlist id="category" runat="server"></asp:dropdownlist>
								...
								<asp:button class="button" id="n_exe" runat="server" Text="执行"></asp:button> 全选<input onClick="CheckAll(this.form)" type="checkbox" name="chkall">
							</td>
						</tr>
						<tr>
							<td width="55%">
